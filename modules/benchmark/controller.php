...
if(isset($_REQUEST['benchmarkId']) && $_REQUEST['benchmarkId'] > 0)
{
	...
	$ThisExerciseId = $Detail->ExerciseId;
        $i++;
            ...
            $html .= '<div style="float:right;margin:10px 20px 10px 0"><input type="button" id="" name="btn" data-mini="true" onClick="SaveTheseResults(\''.$WorkoutId.'_'.$WorkoutTypeId.'\', \''.$Detail->RoutineNo.'_'.$Detail->RoundNo.'_'.$Detail->OrderBy.'_'.$ThisExerciseId.'\');" value="Add Results"/></div>';
            $html .= '</div></form></div><div class="clear"></div></div>';                                

                            }   
           $html.='<div style="float:left;width:65%" id="'.$ThisRoutine.'_timerContainer"></div>';                       
            $html.='<div style="width:30%;float:right;margin:10px 4px 0 0"><input class="buttongroup" id="'.$ThisRoutine.'_ShowHideClock" type="button" onClick="DisplayStopwatch(\'benchmark\', \''.$WorkoutTypeId.'_'.$WorkoutId.'_'.$ThisRoutine.'\');" value="Timer"/></div><div class="clear"></div>';                                                                    
     $html.='</div><br/><br/>';
    //$html.='</form>';
